...
| `331c4d5` | plumb `n_cores` to the growth and mesh steps | It never reached them: `meta_grid` had no such argument and hardcoded `cpu_count()/2` (36 here), `bake_mesh_pickle` likewise, and `growth_mesh` passed no core count -- so `n_cores` in the config did nothing for the two steps that dominate memory and runtime. Each growth worker holds a full voxel grid, so this IS the peak-memory knob. 0/absent keeps the old default. |
| `198e1a1` | clamp the voxel index in `fill_grid` | `is_inside_box` admits `p == extent` while the grid shape is `ceil(extent/box_step)`, so a sample on the far face indexes one past the end when the extent is an integer multiple of the step -- a silent out-of-bounds write, since numba has bounds-checking off. Measure-zero and not observed on real data; guarded because the failure mode is corruption rather than a crash. |
